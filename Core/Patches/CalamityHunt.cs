...
namespace VariousModPatches
{
    public class CalamityHuntFix : ModSystem
    {
        ...
        private static ILHook _unloadHook;
        private static bool _isUnloading = false;

        public override void PostSetupContent()
        {
            var config = ModContent.GetInstance<VariousModPatchesConfig>();
            if (!config.PatchCalamityHunt) return;

            var mod = VersionHelper.GetModAndWarn("CalamityHunt", "CalamityHunt");
            if (mod == null) return;
            ...
            {
                Mod.Logger.Warn("[VariousModPatches] (CalamityHunt) UnloadModContent method not found");
            }

            var targetType = mod.Code.GetType("CalamityHunt.Common.Systems.TileEdgeHighlight");
            if (targetType == null)
            {
                Mod.Logger.Warn("[VariousModPatches] (CalamityHunt) TileEdgeHighlight type not found");
                return;
            }
            ...
            if (method == null)
            {
                Mod.Logger.Warn("[VariousModPatches] (CalamityHunt) CombineTileTargets method not found");
                return;
            }
            ...
            {
                Mod.Logger.Error($"[VariousModPatches] (CalamityHunt) ILHook failed: {ex.Message}");
            }
        }
